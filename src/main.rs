...
impl EditingCursor {
    pub fn from_mouse_position(physically_layouted_line_buffers: &[LineBuffer], mouse_position: (f64, f64)) -> Self {
        ...
        'outer_loop: for (line_index, line_buffer) in physically_layouted_line_buffers.iter().enumerate() {
            ...
            let line_vertical_position = layouted_line.layouted_glyphs.first().unwrap().physical_y_offset.unwrap() as f64;

            if mouse_position.1 <= line_vertical_position && mouse_position.1 > line_vertical_position - line_height as f64 {
...
fn main() -> glib::ExitCode {
    ...
    let application = gtk4::Application::builder().application_id("com.github.ghovax.editex").build();
    ...
    application.connect_activate(|application| {
        ...
        for (button_icon_data, button_action) in [
            ...
            let image_bytes_stream = MemoryInputStream::from_bytes(&glib::Bytes::from_owned(button_icon_data));
        ...
        let drawing_area = Rc::new(DrawingArea::new()); // TODO(ghovax): Just `Rc`?
        drawing_area.set_content_width(1432); // TODO(ghovax): Configure from other stuff.
...

        {
            ...
            single_click_left_mouse_button_gesture.connect_pressed(move |gesture, _, x, y| {
                ...
                log::trace!("The primary mouse button was pressed at {:?} in the drawing area", mouse_position);
        ...
        #[allow(deprecated)]
        let surface = Surface::new_raster_n32_premul((default_window_width * scale_factor, default_window_height * scale_factor)).unwrap();
        log::debug!(
...

        {
            ...
            drawing_area.set_draw_func(move |_drawing_area, cairo_context, width, height| {
                ...
                if surface.as_ref().width() != width * scale_factor || surface.as_ref().height() != height * scale_factor {
                    #[allow(deprecated)]
                    let surface_replacement = Surface::new_raster_n32_premul((width * scale_factor, height * scale_factor)).unwrap();
                ...
                let mut draw_filled_rectangle = |x, y, width, height, color: text::color::Color| {
                    ...
                    canvas.draw_rect(Rect::from_xywh(x as f32, y as f32, width as f32, height as f32), &painting_options);
                };
                    ...
                    match document_element {
                        DocumentElement::Line { anchor_point, spans } => {
                            ...
                            for glyph in layouted_line.layouted_glyphs.iter_mut() {
                                ...
                                rasterizer_cache.with_pixels(&mut font_system, glyph.cache_key.unwrap(), glyph_color, |x, y, color| {
                ...
                for ((line_index, line_buffer), document_element) in layouted_line_buffers.iter().enumerate().zip(document_elements.iter())
                {
                    match document_element {
                        DocumentElement::Line { anchor_point, .. } => {
                            ...
                            if let Some((cursor_glyph_index, cursor_glyph_horizontal_offset)) = retrieve_cursor_position() {
                                ...
                                log::trace!(
                                    ...
                                    (x + anchor_point.0 as i32, (anchor_point.1 - layouted_line.maximum_y_reach) as i32)
                                );
                ...
                for (line_buffer, document_element) in layouted_line_buffers.iter().zip(document_elements.iter()) {
                    let layouted_line = line_buffer.layouted_line.as_ref().unwrap();
                    ...
                    match document_element {
                        DocumentElement::Line { anchor_point, .. } => {
                            ...
                            let x_origin = layouted_line.layouted_glyphs.first().unwrap().physical_x_offset.unwrap() as f32;
